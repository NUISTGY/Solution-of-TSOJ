...
#include <list>
using namespace std;
inline void judgement(char time[],int*hours,int*minutes);
inline void show(int *newTime,int hours,int minutes);
inline void circle(int n);
inline int difference(char* zone1,char* zone2);
inline void start();
inline int difference(char* zone1,char* zone2){//计算两个时区之间的时差，以分钟为单位
...
inline void circle(int n){
    for(int i=0;i<n;i++){      //对每组输入数据
        char time[9];               //输入的时间
        int hours,minutes;          //转换成整数
        cin>>time;                  //读入时间
        judgement(time,&hours,&minutes);
